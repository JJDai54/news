...
function b_news_top_show($options)
{
    ...
    if (2 == $options[8]) { // Tabbed view ********************************************************************************************
        ...
        if (0 == $currenttab && $usespotlight) { // Spotlight or not ?
            ...
            if (1 == $tmpstory->votes()) {
                $spotlight['number_votes'] = _MD_NEWS_ONEVOTE;
            } else {
                $spotlight['number_votes'] = sprintf(_MD_NEWS_NUMVOTES, $tmpstory->votes());
            }

            $spotlight['votes_with_text'] = sprintf(_MD_NEWS_NUMVOTES, $tmpstory->votes());
            ...
            if (count($stories) > 0) {
                foreach ($stories as $key => $story) {
                    ...
                    $news['topic_color_set'] = news_get_color_set($story->topic_color_set);
            ...
            if ($tabscount > 0) {
                ...
                $smallheader[] = sprintf('%u %s', $stats['count'], _MD_NEWS_ARTICLE_S);
                $smallheader[] = sprintf('%u %s', $stats['reads'], _MD_NEWS_VIEWS);
                if (count($stories) > 0) {
                    foreach ($stories as $key => $story) {
                        $news  = [];
                        $title = $story->title();
                        ...
                        if (1 == $story->votes()) {
                            $news['number_votes'] = _MD_NEWS_ONEVOTE;
                        } else {
                            $news['number_votes'] = sprintf(_MD_NEWS_NUMVOTES, $story->votes());
                        }
                        if ($infotips > 0) {
                            $news['infotips'] = ' title="' . NewsUtility::makeInfotips($story->hometext()) . '"';
                        } else {
        ...
        $block['lang_reads'] = _MD_NEWS_VIEWS; // reads
        // Default values
        ...
        foreach ($stories as $key => $story) {
            ...
            if ((0 == $options[5]) && (1 == $options[4])
                ...
                if ($visible) {
                    ...
                    $spotlight['topic_color_set'] = news_get_color_set($story->topic_color_set);
                    // Added, topic's image and description
        ...
        if (!isset($spotlight) && $options[4]) {
            ...
            if (true === $block['use_spotlight']) {
                ...
                $spotlight['topic_color_set'] = news_get_color_set($spotlightArticle->topic_color_set);
...
function b_news_top_edit($options)
{
    ...
    $form .= '<br><br>' . _MB_NEWS_DISP . "&nbsp;<select name='options[]'>";

    $form .= "<option value='1' ";
    if (1 == $options[8]) {
        $form .= 'selected';
    }
    $form .= '>' . _MB_NEWS_VIEW_TYPE1 . "</option>";
    //---------------------------------------------------------
    $form .= "<option value='2' ";
    if (2 == $options[8]) {
        $form .= 'selected';
    }
    $form .= '>' . _MB_NEWS_VIEW_TYPE2 . '</option>';
    //---------------------------------------------------------
    $form .= "<option value='3' ";
    if (3 == $options[8]) {
        $form .= 'selected';
