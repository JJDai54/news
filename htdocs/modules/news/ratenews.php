...
if (!empty($_POST['submit'])) { // The form was submited
    ...
    if ('--' == $rating) {
        redirect_header(XOOPS_URL . '/modules/news/ratenews.php?storyid=' . $storyid, 4, _MD_NEWS_NORATING);
    }
    ...
    if (0 != $ratinguser) {
        ...
        while (list($ratinguserDB) = $xoopsDB->fetchRow($result)) {
            if ($ratinguserDB == $ratinguser) {
                redirect_header(XOOPS_URL . '/modules/news/article.php?storyid=' . $storyid, 4, _MD_NEWS_CANTVOTEOWN);
            }
        }
        ...
        while (list($ratinguserDB) = $xoopsDB->fetchRow($result)) {
            if ($ratinguserDB == $ratinguser) {
                redirect_header(XOOPS_URL . '/modules/news/article.php?storyid=' . $storyid, 4, _MD_NEWS_VOTEONCE);
            }
        }
    } else {
    ...
    $ratemessage = _MD_NEWS_VOTEAPPRE . '<br>' . sprintf(_MD_NEWS_THANKYOU, $xoopsConfig['sitename']);
    ...
    $xoopsTpl->assign('lang_voteonce', _MD_NEWS_VOTEONCE);
    $xoopsTpl->assign('lang_ratingscale', _MD_NEWS_RATINGSCALE);
    $xoopsTpl->assign('lang_beobjective', _MD_NEWS_BEOBJECTIVE);
    $xoopsTpl->assign('lang_donotvote', _MD_NEWS_DONOTVOTE);
    $xoopsTpl->assign('lang_rateit', _MD_NEWS_RATEIT);
    $xoopsTpl->assign('lang_cancel', _CANCEL);
    $xoopsTpl->assign('xoops_pagetitle', $title . ' - ' . _MD_NEWS_RATETHISNEWS . ' - ' . $xoopsModule->name('s'));
