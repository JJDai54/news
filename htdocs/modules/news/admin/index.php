...
function newSubmissions()
{
    ...
    if (count($storyarray) > 0) {
        ...
        echo "<img onclick=\"toggle('toptable'); toggleIcon('toptableicon');\" id='topnewsubicon' name='topnewsubicon' src='" . $pathIcon16 . "/close12.gif' alt=''></a>&nbsp;" . _AM_NEWS_NEWSUB . '</h4>';
        echo "<div id='newsub'>";
        echo '<br>';
...
function autoStories()
{
    ...
    if (count($storyarray) > 0) {
        ...
        echo "<img onclick=\"toggle('toptable'); toggleIcon('toptableicon');\" id='topautostories' name='topautostories' src='" . $pathIcon16 . "/close12.gif' alt=''></a>&nbsp;" . _AM_NEWS_AUTOARTICLES . '</h4>';
...
function lastStories()
{
    ...
    echo "<img onclick=\"toggle('toptable'); toggleIcon('toptableicon');\" id='toplaststories' name='toplaststories' src='" . $pathIcon16 . "/close12.gif' alt=''></a>&nbsp;" . sprintf(_AM_NEWS_LAST10ARTS, NewsUtility::getModuleOption('storycountadmin')) . '</h4>';
    ...
    foreach ($storyarray as $eachstory) {
        ...
        echo "<td align='center'><b>" . $eachstory->storyid() . "</b>
            ...
            <a href='" . XOOPS_URL . '/modules/news/admin/index.php?op=delete&amp;storyid=' . $eachstory->storyid() . "'><img src='" . $pathIcon16 . "/delete.png' title='" . _AM_NEWS_DELETE . "'></a>";
    ...
    echo "<form action='index.php' method='get'>" . _AM_NEWS_STORYID . " <input type='text' name='storyid' size='10'>
        <select name='op'>
            <option value='edit' selected>" . _AM_NEWS_EDIT . "</option>
            <option value='delete'>" . _AM_NEWS_DELETE . "</option>
        </select>
        <input type='hidden' name='returnside' value='1'>
        <input type='submit' value='" . _AM_NEWS_GO . "'>
        </form>
    </div>";
    echo '</div><br>';
}
...
function expStories()
{
    ...
    if (count($storyarray) > 0) {
        ...
        echo "<img onclick=\"toggle('toptable'); toggleIcon('toptableicon');\" id='topexpstories' name='topexpstories' src='" . $pathIcon16 . "/close12.gif' alt=''></a>&nbsp;" . _AM_NEWS_EXPARTS . '</h4>';
        ...
        echo "<form action='index.php' method='get'>
            " . _AM_NEWS_STORYID . " <input type='text' name='storyid' size='10'>
            <select name='op'>
                <option value='edit' selected>" . _AM_NEWS_EDIT . "</option>
                <option value='delete'>" . _AM_NEWS_DELETE . "</option>
            </select>
...
function topicsmanager()
{
    ...
    $uploadfolder   = sprintf(_AM_NEWS_UPLOAD_WARNING, XOOPS_URL . '/uploads/news/image');
    ...
    //echo "<img onclick=\"toggle('toptable'); toggleIcon('toptableicon');\" id='toptopicsmanager' name='toptopicsmanager' src='" . $pathIcon16."/close12.gif' alt=''></a>&nbsp;"._AM_NEWS_TOPICSMNGR . ' (' . $totaltopics . ')'."</h4>";
    ...
    echo "<table width='100%' cellspacing='1' cellpadding='3' border='0' class='outer'><tr class='bg3'><th align='center'>"
         ...
         . _AM_NEWS_TOPICNAME
         . "</th><th align='center'>"
         . _AM_NEWS_PARENTTOPIC
         . "</th><th align='center'>"
         . _AM_NEWS_SUB_MENU_YESNO
         . "</th>"
         ."<th align='center'>" . _AM_NEWS_WEIGHT . '</th>'
         ."<th align='center'>" . _AM_NEWS_COLOR_SET . '</th>'
         ."<th align='center'>" . _AM_NEWS_ACTION  . '</th>'
         .'</tr>';
    if (is_array($topics_arr) && $totaltopics) {
        ...
        while ($ok) {
            if ($tmpcpt < $totaltopics) {
                $action_edit   = '<a href=index.php?op=topicsmanager&amp;topic_id=' . $topics_arr[$tmpcpt]['topic_id'] . '><img src=' . $pathIcon16 . 'edit.png title=' . _AM_NEWS_EDIT . '></a>';
                $action_delete = '<a href=index.php?op=delTopic&amp;topic_id=' . $topics_arr[$tmpcpt]['topic_id'] . '><img src=' . $pathIcon16 . '/delete.png title=' . _AM_NEWS_DELETE . "'></a>";
                ...
                $output = $output
                          ...
                          . "<td align='right'>" . $topics_arr[$tmpcpt]['topic_weight'] . "</td>"
                          . "<td align='center'>" . $topics_arr[$tmpcpt]['topic_color_set'] . "</td>"
                          . "<td align='center'>" . $action_edit . $action_delete . '</td>'
                          . '</tr>';
            } else {
                $ok = false;
            }
    ...
    if ($topic_id > 0) {
        ...
        $topic_weight       = $xtmod->topic_weight();
        ...
        $btnlabel        = _AM_NEWS_MODIFY;
        $parent          = $xtmod->topic_pid();
        $formlabel       = _AM_NEWS_MODIFYTOPIC;
        $submenu         = $xtmod->menu();
        $topic_frontpage = $xtmod->topic_frontpage();
        $topic_color     = $xtmod->topic_color();
        $topic_color_set     = $xtmod->topic_color_set();
        unset($xtmod);
    } else {
        $topic_title       = '';
        $topic_weight      = 0;
        ...
        $btnlabel          = _AM_NEWS_ADD;
        $parent            = -1;
        $submenu           = 0;
        $topic_rssfeed     = '';
        $formlabel         = _AM_NEWS_ADD_TOPIC;
        $topic_color       = '000000';
        $topic_color_set       = '';
    }
    ...
    $sform->addElement(new XoopsFormText(_AM_NEWS_TOPICNAME, 'topic_title', 50, 255, $topic_title), true);
    $editor = NewsUtility::getWysiwygForm(_AM_NEWS_TOPIC_DESCR, 'topic_description', $topic_description, 15, 60, 'hometext_hidden');
    if ($editor) {
    ...
    require_once XOOPS_ROOT_PATH . '/modules/news/class/class.newstopic.php';
    $xt = new NewsTopic();
    $sform->addElement(new XoopsFormLabel(_AM_NEWS_PARENTTOPIC, $xt->makeMyTopicSelBox(1, $parent, 'topic_pid', '', false)));

    //poids
    $sform->addElement(new XoopsFormText(_AM_NEWS_WEIGHT, 'topic_weight', 25, 25, $topic_weight), false);

    // Topic's color
    // Code stolen to Zoullou, thank you Zoullou ;-)
    $inpColor = new \XoopsFormColorPicker(_AM_NEWS_COLOR, 'topic_color', $topic_color);
    $sform->addElement($inpColor);
/*
    ...
    $color_values = [
        ...
        '003366','0033CC','0033FF','006600','006633','006666','006699','0066CC',
...
*/
     ...
     $selectFormColorSet = new \XoopsFormSelect(_AM_NEWS_COLOR_SET, 'topic_color_set', $topic_color_set);
     $selectFormColorSet->addOptionArray(news_get_css_color());
     $selectFormColorSet->setDescription(_AM_NEWS_COLOR_SET_DESC);
     $sform->addElement($selectFormColorSet);

    // Sub menu ?
    $sform->addElement(new XoopsFormRadioYN(_AM_NEWS_SUB_MENU, 'submenu', $submenu, _YES, _NO));
    $sform->addElement(new XoopsFormRadioYN(_AM_NEWS_PUBLISH_FRONTPAGE, 'topic_frontpage', $topic_frontpage, _YES, _NO));
    // Unused for this moment... sorry
    //$sform->addElement(new XoopsFormText(_AM_NEWS_RSS_URL, 'topic_rssfeed', 50, 255, $topic_rssfeed), false);
    // ********** Picture
    $imgtray = new XoopsFormElementTray(_AM_NEWS_TOPICIMG, '<br>');

    $imgpath      = sprintf(_AM_NEWS_IMGNAEXLOC, 'uploads/news/image/');
    ...
    $fileseltray->addElement(new XoopsFormFile(_AM_NEWS_TOPIC_PICTURE, 'attachedfile', NewsUtility::getModuleOption('maxuploadsize')), false);
    ...
    if ($topic_id > 0) { // Edit mode
        ...
        $groups_news_can_approve_checkbox = new XoopsFormCheckBox(_AM_NEWS_APPROVEFORM, 'groups_news_can_approve[]', $groups_ids);
    } else { // Creation mode
        $groups_news_can_approve_checkbox = new XoopsFormCheckBox(_AM_NEWS_APPROVEFORM, 'groups_news_can_approve[]', $full_list);
    }
    ...
    if ($topic_id > 0) { // Edit mode
        ...
        $groups_ids                      = array_values($groups_ids);
        $groups_news_can_submit_checkbox = new XoopsFormCheckBox(_AM_NEWS_SUBMITFORM, 'groups_news_can_submit[]', $groups_ids);
    } else { // Creation mode
        $groups_news_can_submit_checkbox = new XoopsFormCheckBox(_AM_NEWS_SUBMITFORM, 'groups_news_can_submit[]', $full_list);
    }
    ...
    if ($topic_id > 0) { // Edit mode
        ...
        $groups_ids                    = array_values($groups_ids);
        $groups_news_can_view_checkbox = new XoopsFormCheckBox(_AM_NEWS_VIEWFORM, 'groups_news_can_view[]', $groups_ids);
    } else { // Creation mode
        $groups_news_can_view_checkbox = new XoopsFormCheckBox(_AM_NEWS_VIEWFORM, 'groups_news_can_view[]', $full_list);
    }
...
function modTopicS()
{
    ...
    if ((int)$_POST['topic_pid'] == (int)$_POST['topic_id']) {
        redirect_header('index.php?op=topicsmanager', 2, _AM_NEWS_ADD_TOPIC_ERROR1);
    }
    $xt->setTopicPid((int)$_POST['topic_pid']);
    if (empty($_POST['topic_title'])) {
        redirect_header('index.php?op=topicsmanager', 2, _AM_NEWS_ERRORTOPICNAME);
    }
    if (isset($_SESSION['items_count'])) {
        $_SESSION['items_count'] = -1;
    }
    $xt->setTopicTitle($_POST['topic_title']);
    $xt->setTopicWeight($_POST['topic_weight']);
    ...
    $xt->setTopic_color_set($_POST['topic_color_set']);
        ...
        if (xoops_trim('' !== $fldname)) {
            ...
            if ($uploader->fetchMedia($_POST['xoops_upload_file'][0])) {
                if ($uploader->upload()) {
                    ...
                    echo _AM_NEWS_UPLOAD_ERROR . ' ' . $uploader->getErrors();
                }
            } else {
    ...
    NewsUtility::updateCache();
    redirect_header('index.php?op=topicsmanager', 1, _AM_NEWS_DBUPDATED);
}
...
function delTopic()
{
    ...
    if (!isset($_POST['ok'])) {
        ...
        xoops_confirm(['op' => 'delTopic', 'topic_id' => (int)$_GET['topic_id'], 'ok' => 1], 'index.php', _AM_NEWS_WAYSYWTDTTAL . '<br>' . $xt->topic_title('S'));
    } else {
        xoops_cp_header();
...
function addTopic()
{
    ...
    if (!$xt->topicExists($topicpid, $_POST['topic_title'])) {
        ...
        redirect_header('index.php?op=topicsmanager', 2, _AM_NEWS_ADD_TOPIC_ERROR);
    }
}
...
function getStats()
{
    ...
    echo "<table border='0' width='100%'><tr class='bg3'><th align='center'>" . _AM_NEWS_TOPIC . "</th><th align='center'>" . _MD_NEWS_ARTICLE_S . '</th><th>' . _MD_NEWS_VIEWS . '</th><th>' . _AM_NEWS_UPLOAD_ATTACHFILE . '</th><th>' . _AM_NEWS_EXPARTS . '</th><th>' . _AM_NEWS_STATS1 . '</th></tr>';
    ...
    echo "<table border='0' width='100%'><tr class='bg3'><th align='center'>" . _AM_NEWS_TOPIC . "</th><th align='center'>" . _AM_NEWS_TITLE . '</th><th>' . _AM_NEWS_POSTER . '</th><th>' . _MD_NEWS_VIEWS . "</th></tr>\n";
    ...
    echo "<table border='0' width='100%'><tr class='bg3'><th align='center'>" . _AM_NEWS_TOPIC . "</th><th align='center'>" . _AM_NEWS_TITLE . '</th><th>' . _AM_NEWS_POSTER . '</th><th>' . _MD_NEWS_RATING . "</th></tr>\n";
    ...
    echo "<table border='0' width='100%'><tr class='bg3'><th>" . _AM_NEWS_POSTER . '</th><th>' . _MD_NEWS_VIEWS . "</th></tr>\n";
    ...
    echo "<table border='0' width='100%'><tr class='bg3'><th>" . _AM_NEWS_POSTER . '</th><th>' . _MD_NEWS_RATING . "</th></tr>\n";
    ...
    echo "<table border='0' width='100%'><tr class='bg3'><th>" . _AM_NEWS_POSTER . '</th><th>' . _AM_NEWS_STATS11 . "</th></tr>\n";
...
function getMetagen()
{
    ...
    $button_tray = new XoopsFormElementTray('', '');
    $submit_btn  = new XoopsFormButton('', 'post', _AM_NEWS_MODIFY, 'submit');
    $button_tray->addElement($submit_btn);
    $sform->addElement($button_tray);
    $sform->display();
    ...
    $remove_btn = new XoopsFormButton('', 'go', _AM_NEWS_DELETE, 'submit');
    ...
    $add_btn = new XoopsFormButton('', 'go', _AM_NEWS_ADD, 'submit');
...
function saveMetagenBlackList()
{
    ...
    if (isset($_POST['go']) && _AM_NEWS_DELETE == $_POST['go']) {
        foreach ($_POST['blacklist'] as $black_id) {
            $blacklist->delete($black_id);
        }
        $blacklist->store();
    } else {
        if (isset($_POST['go']) && _AM_NEWS_ADD == $_POST['go']) {
    ...
    redirect_header('index.php?op=metagen', 0, _AM_NEWS_DBUPDATED);
}
...
switch ($op) {
    ...
    case 'newarticle':
        ...
        expStories();
        echo '<br>';
        echo '<h4>' . _AM_NEWS_POSTNEWARTICLE . '</h4>';
        $type         = 'admin';
        $title        = '';
        $topicdisplay = 0;
        $topicalign   = 'R';
    ...
    case 'delete':
        ...
        if (!empty($_POST['ok'])) {
            if (empty($storyid)) {
                redirect_header('index.php?op=newarticle', 2, _AM_NEWS_EMPTYNODELETE);
            }
            ...
            NewsUtility::updateCache();
            redirect_header('index.php?op=newarticle', 1, _AM_NEWS_DBUPDATED);
        } else {
            $story = new NewsStory($storyid);
            xoops_cp_header();
            echo '<h4>' . _AM_NEWS_CONFIG . '</h4>';
            xoops_confirm(['op' => 'delete', 'storyid' => $storyid, 'ok' => 1], 'index.php', _AM_NEWS_RUSUREDEL . '<br>' . $story->title());
        }
        break;
    ...
    case 'verifydb':
        ...
        redirect_header('index.php', 3, _AM_NEWS_DBUPDATED);
        break;
