...
class MyXoopsStory
{
    ...
    public function store($approved = false)
    {
        ...
        $subtitle = $myts->addSlashes($this->subtitle);
...
//            echo "===> {$key} ===> {$value}<br>";
    ...
    public function subtitle($format = 'Show')
    {
        $myts   = MyTextSanitizer::getInstance();
        $smiley = 1;
        if ($this->nosmiley()) {
        ...
        switch ($format) {
            case 'Show':
            case 'Edit':
                $subtitle = $myts->htmlSpecialChars($this->subtitle);
                break;
            case 'Preview':
            case 'InForm':
                $subtitle = $myts->htmlSpecialChars($myts->stripSlashesGPC($this->subtitle));
                break;
        }
