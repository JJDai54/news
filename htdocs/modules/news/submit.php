...
// ***************************************************************
        ...
        if ($approveprivilege) {
            ...
            if (!$story->published() && $approve) {
                $story->setPublished(NewsUtility::news_getAroundTime());  //   time()
            }
            if (!$story->expired()) {
                $story->setExpired(0);
            }
                    ...
                    if (xoops_trim('' !== $fldname)) {
                        ...
                        if ($uploader->fetchMedia($_POST['xoops_upload_file'][0])) {
                            if ($uploader->upload()) {
                                ...
                                if (!$sfiles->store()) {
                                    echo _AM_NEWS_UPLOAD_DBERROR_SAVE;
                                }
                            } else {
                                echo _AM_NEWS_UPLOAD_ERROR . ' ' . $uploader->getErrors();
                            }
        ...
        if (!$returnside) {
            //redirect_header(XOOPS_URL . '/modules/news/index.php', 2, _MD_NEWS_THANKS);
            redirect_header(XOOPS_URL . '/modules/news/article.php?storyid='.$storyid, 2, _MD_NEWS_THANKS);

        } else {
            redirect_header(XOOPS_URL . '/modules/news/admin/index.php?op=newarticle', 2, _MD_NEWS_THANKS);
        }
        break;
