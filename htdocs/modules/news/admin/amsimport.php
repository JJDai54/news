            ...
            while ($article = $db->fetchArray($result)) {
                ...
                if ($use_extlinks) {
                    ...
                    while ($link = $db->fetchArray($result7)) {
                        if ('' == trim($links)) {
                            $links = "\n\n" . _AMS_MD_NEWS_RELATEDARTICLES . "\n\n";
                        }
                        $links .= _AMS_MD_NEWS_EXTERNALLINK . ' [url=' . $link['link_link'] . ']' . $link['link_title'] . '[/url]' . "\n";
                    }
                }
                ...
                if ($use_forum && 0 != $one_amstopic['forum_id']) {
                    $forum = "\n\n" . '[url=' . XOOPS_URL . '/modules/newbb/viewforum.php?forum=' . $one_amstopic['forum_id'] . ']' . _AMS_AM_NEWS_LINKEDFORUM . '[/url]' . "\n";
                }
