...
$xoopsTpl->assign('xoops_pagetitle', $myts->htmlSpecialChars(_MD_NEWS_NEWSARCHIVES) . $pgtitle . ' - ' . $xoopsModule->name('s'));
...
if (0 != $fromyear && 0 != $frommonth) {
    ...
    $xoopsTpl->assign('lang_articles', _MD_NEWS_ARTICLE_S);
    $xoopsTpl->assign('currentmonth', $months_arr[$frommonth]);
    $xoopsTpl->assign('currentyear', $fromyear);
    $xoopsTpl->assign('lang_actions', _MD_NEWS_ACTIONS);
    $xoopsTpl->assign('lang_date', _MD_NEWS_DATE);
    $xoopsTpl->assign('lang_views', _MD_NEWS_VIEWS);
    ...
    if (is_array($storyarray) && $count > 0) {
        foreach ($storyarray as $article) {
            ...
            $story['mail_link']  = 'mailto:?subject=' . sprintf(_MD_NEWS_INTARTICLE, $xoopsConfig['sitename']) . '&amp;body=' . sprintf(_MD_NEWS_INTARTFOUND, $xoopsConfig['sitename']) . ':  ' . XOOPS_URL . '/modules/' . $xoopsModule->dirname() . '/article.php?storyid=' . $article->storyid();
            $xoopsTpl->append('stories', $story);
        }
    }
    $xoopsTpl->assign('lang_printer', _MD_NEWS_PRINTERFRIENDLY);
    $xoopsTpl->assign('lang_sendstory', _MD_NEWS_SENDSTORY);
    $xoopsTpl->assign('lang_storytotal', sprintf(_MD_NEWS_THEREAREINTOTAL, $count));
} else {
    $xoopsTpl->assign('show_articles', false);
}

$xoopsTpl->assign('lang_newsarchives', _MD_NEWS_NEWSARCHIVES);
