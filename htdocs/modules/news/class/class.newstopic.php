...
//exit ($sql);
     ...
     * @param $value
     */
    public function setTopic_color_set($value)
    {
        $this->topic_color_set = $value;
    }
    ...
    public function store()
    {
        ...
        $topic_color_set   = $this->topic_color_set;
        ...
        $topic_weight = (int)$this->topic_weight;
        $insert          = false;
        if (empty($this->topic_id)) {
            $insert         = true;
            $this->topic_id = $this->db->genId($this->table . '_topic_id_seq');
            $sql            = sprintf(
                "INSERT INTO %s (topic_id, topic_pid, topic_imgurl, topic_title,topic_weight, menu, topic_description, topic_frontpage, topic_rssurl, topic_color, topic_color_set)"
                 . " VALUES (%u, %u, '%s', '%s', %u, '%s',%d,  %d, '%s', '%s', '%s')",
                $this->table,
                (int)$this->topic_id,
                (int)$this->topic_pid,
                $imgurl,
            ...
            $sql = sprintf(
                "UPDATE %s SET topic_pid = %u, topic_imgurl = '%s', topic_title = '%s', topic_weight=%d,  menu=%d, topic_description='%s', topic_frontpage=%d, topic_rssurl='%s', topic_color='%s', topic_color_set='%s' WHERE topic_id = %u",
                $this->table,
                (int)$this->topic_pid,
                $imgurl,
                $title,
...
//         return $topic_color;
     ...
     * @return mixed
     */
    public function topic_color_set($format = 'S')
    {
//         $myts = MyTextSanitizer::getInstance();
//         switch ($format) {
//             case 'S':
//                 $topic_color_set = $myts->displayTarea($this->topic_color_set);
//                 break;
//             case 'P':
//                 $topic_color_set = $myts->previewTarea($this->topic_color_set);
//                 break;
//             case 'F':
//             case 'E':
//                 $topic_color_set = $myts->htmlSpecialChars($this->topic_color_set);
//                 break;
//         }
//
//         return $topic_color_set;
        return $this->topic_color_set;
    }
    ...
    //$perms=true;
    //echo "===>perms = {$perms}<br>";
        $sql = 'SELECT topic_id, topic_pid, topic_title, topic_color, topic_color_set FROM ' . $this->table . ' WHERE 1 ' ;
        if ($frontpage) {
        ...
        if ($perms) {
            ...
            //echo "===>topicsids<pre>" . print_r($topicsids, true) . "</pre>";
        ...
        while ($myrow = $this->db->fetchArray($result)) {
            $ret[$myrow['topic_id']] = [
                ...
                'color_set' => news_get_color_set($myrow['topic_color_set'])
            ];
        }
    //echo "getTopicsList===>->sql = {$sql}<br>";
//exit;
        return $ret;
    }
